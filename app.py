...
def register():
    if request.method == "POST":
        ...
        with sqlite3.connect(DATABASE) as con:
            c = con.cursor()
            c.execute(
                "INSERT INTO users (full_name, contact_number, hashed_password, role) VALUES (?, ?, ?, ?)",
...
def index():
    ...
    curr_room = request.args.get("room")
    with sqlite3.connect(DATABASE) as con:
        c = con.cursor()
...
def join():
    ...
    with sqlite3.connect(DATABASE) as con:
        ...
        if not existing:
            ...
            c.execute(
                "INSERT INTO rooms (room_name, created_on, created_by_user) VALUES (?, ?, ?)",
    ...
    if new_room_created:
        socketio.emit(
            "room_created", {"room": room, "creator_id": user_id}, namespace="/"
        )
...
def leave():
    ...
    with sqlite3.connect(DATABASE) as con:
        c = con.cursor()
        c.execute(
            "DELETE FROM room_members WHERE user_id=? AND room_id=(SELECT room_id FROM rooms WHERE room_name=?)",
            (user_id, room),
        )
        con.commit()
...
def delete_room():
    ...
    with sqlite3.connect(DATABASE) as con:
        c = con.cursor()
        row = c.execute(
            "SELECT room_id, created_by_user FROM rooms WHERE room_name=?", (room,)
        ).fetchone()
...
def handle_message(data):
    ...
    with sqlite3.connect(DATABASE) as con:
        c = con.cursor()
        c.execute(
            "INSERT INTO chats (message_text, sent_at, sender_id, room_id) VALUES (?, ?, ?, (SELECT room_id FROM rooms WHERE room_name=?))",
